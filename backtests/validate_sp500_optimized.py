"""VALIDATE: Optimized Momentum Breakout — SP500 1-min.

Grid search found SL=1.2, RR=1.5 as best config:
  +28.19% return, 14.2% DD, 4/4 positive quarters ✅
Lower RR gives higher win rate (42.5%) which offsets lower per-trade payout.
No trend filter needed — hurts returns on SP500.
"""
import csv, math
from pathlib import Path

DATA = "data/market_cache/fmp_SP500_1min.csv"
RESULTS = Path(__file__).resolve().parent / "validate_sp500_optimized.txt"
...
log("=" * 70)
log("OPTIMIZED — Momentum Breakout (SP500 1-min)")
log("=" * 70)
...
# Tight RR configs perform best on SP500
configs = [
...
log("")
log("─" * 60)
log(f"WALK-FORWARD (4 quarters, SL={best_sl} RR={best_rr}):")
qf = n // 4
...
log(f"  Best config (SL={best_sl}×, RR={best_rr}×): {r_full['ret']}% on SP500 1-min")
